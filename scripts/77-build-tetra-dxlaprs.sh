#!/usr/bin/env bash
set -euo pipefail

# read and export dependencies
# shellcheck disable=SC1091
source /build.env

SRC_URL="http://oe5dxl.hamspirit.at:8025/aprs/c/"
PKG_NAME="dxlaprs-tetra"

if [ "${BUILD_TETRA_DXLAPRS:-}" == "y" ]; then
	log suc "Building dxlAPRS TETRA tools (tetrarx)..."
	export DEBIAN_FRONTEND=noninteractive

	rm -rf tetra-dxlaprs
	mkdir tetra-dxlaprs
	pushd tetra-dxlaprs

	# Download all files from the dxlAPRS toolchain server
	log inf "Downloading sources from ${SRC_URL}..."
	wget -nv --no-proxy -r -np -nd -l1 "${SRC_URL}" || {
		log err "Failed to download sources from ${SRC_URL}"
		exit 1
	}

	# Download tetra/ subdirectory sources (needed by tetrarx)
	mkdir -p tetra
	wget -nv --no-proxy -r -np -nd -l1 -P tetra "${SRC_URL}tetra/" || {
		log err "Failed to download tetra/ sources from ${SRC_URL}tetra/"
		exit 1
	}

	# Fix osic.c: add explicit casts to silence -Werror=implicit on newer GCC
	sed -i 's/return ctime(time0);/return (int32_t)ctime((time_t*)time0);/' osic.c

	# Apply Makefile patch to add tetrarx build target
	patch -p0 < /scripts/patches/tetra-dxlaprs.patch || {
		log err "Failed to apply tetra-dxlaprs.patch"
		exit 1
	}

	make tetrarx

	pkg_version="1.0.$(date +%Y%m%d)"
	debian_dir="debian"
	rm -rf "${debian_dir}"
	mkdir -p "${debian_dir}/source"
	changelog_date=$(LC_ALL=C date -Ru)

	cat > "${debian_dir}/changelog" <<EOF
${PKG_NAME} (${pkg_version}) unstable; urgency=medium

  * Automated build from ${SRC_URL}

 -- OpenWebRX DEB Builder <builder@openwebrx>  ${changelog_date}
EOF

	cat > "${debian_dir}/control" <<EOF
Source: ${PKG_NAME}
Section: hamradio
Priority: optional
Maintainer: OpenWebRX DEB Builder <builder@openwebrx>
Build-Depends: debhelper (>= 13)
Standards-Version: 4.6.2
Homepage: http://oe5dxl.hamspirit.at:8025/aprs/c/
Rules-Requires-Root: no

Package: ${PKG_NAME}
Architecture: any
Depends: \${shlibs:Depends}, \${misc:Depends}
Description: TETRA tools from the dxlAPRS Toolchain by Christian Rabler (OE5DXL)
 This package provides tetrarx, part of the dxlAPRS toolchain
 created by Christian Rabler (OE5DXL).
 .
 tetrarx is a TETRA IQ demodulator with AXUDP interface supporting TETRA
 packet decoding from SDR IQ input.
 .
 Project homepage: http://oe5dxl.hamspirit.at:8025/aprs/c/
EOF

	cat > "${debian_dir}/compat" <<'EOF'
13
EOF

	cat > "${debian_dir}/source/format" <<'EOF'
3.0 (native)
EOF

	cat > "${debian_dir}/rules" <<'EOF'
#!/usr/bin/make -f
export DH_VERBOSE := 1

%:
	dh $@

override_dh_auto_clean:
	@true

override_dh_auto_configure:
	@true

override_dh_auto_build:
	@true

override_dh_auto_install:
	dh_install
EOF
	chmod +x "${debian_dir}/rules"

	cat > "${debian_dir}/${PKG_NAME}.install" <<'EOF'
tetrarx usr/bin/
EOF

	dpkg-buildpackage -b -us -uc -j"$(nproc --ignore=4)"
	popd

	mkdir -p "${OUTPUT_DIR}"
	shopt -s nullglob
	debs=(${PKG_NAME}_*.deb)
	shopt -u nullglob
	if [ ${#debs[@]} -eq 0 ]; then
		log err "dxlaprs-tetra deb package missing"
		exit 1
	fi
	for deb in "${debs[@]}"; do
		cp "$deb" "${OUTPUT_DIR}/"
	done

	rm -f ${PKG_NAME}_*
	rm -rf tetra-dxlaprs
fi
